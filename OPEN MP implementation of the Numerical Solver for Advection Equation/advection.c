...
int main ( int argc, char *argv[] ){
  ...
  // Take input for the number of threads to use
  printf("Please Enter number of threads you wish to use for this solver: ");
  int num_of_threads;
  ...
  for(int step = 0; step<Nsteps; step++){
    ...
    for(int stage=0; stage<tstep.Nstage; stage++){
      ...
      RhsQ(&advc, &tstep, stage , num_of_threads); 
    }
  ...
  printf("Time taken to find the solution is: %f\n",elapsed_time);

  infinite_norm(&advc,num_of_threads);
...
/* ************************************************************************** */
void RhsQ(solver_t *solver, tstep_t *tstep, int stage , int num_of_threads){

mesh_t *msh = &solver->msh;

// Start of parallel region
#pragma omp parallel for num_threads(num_of_threads) default(none) shared(solver, tstep, msh ,stage)
for(int id = 0; id < (msh->NX * msh->NY); id++){
      int j = id / msh->NX;
      int i = id % msh->NX;
      int idn = (j*msh->NX) + i;
...
void infinite_norm(solver_t *solver, int num_of_threads) {
  mesh_t *msh = &solver->msh;
  double global_rmax = solver->q[0];
  double global_cmax = solver->q[0];
  double global_amax = solver->q[0];
  double start_time;
  double end_time;
  double elapsed_time;

  //                                       SERIAL IMPLEMENTATION
  // for (int i = 0; i < msh->NX * msh->NY; ++i) {
  //       if (solver->q[i] > global_max) {
  //           global_max = solver->q[i];
  //       }
  //   }
  
  start_time = omp_get_wtime();

  #pragma omp parallel num_threads(num_of_threads) default(none) shared(msh, solver, global_rmax )
  {
    //                              IMPLEMENTING REDUCTION PARALLELIZATION

    #pragma omp for reduction(max:global_rmax)
    for(int i = 0; i < msh->NX * msh->NY; ++i) {
      if (solver->q[i] > global_rmax) {
        global_rmax = solver->q[i];
      }
    }
  ...
  end_time = omp_get_wtime();
  elapsed_time = end_time - start_time;
  printf("Time taken to find the infinite norm using Reduction is: %f\n",elapsed_time);
  
  ////////////////////////////////////////////////////////////////////////////////////////////////////////
  start_time = omp_get_wtime();
  #pragma omp parallel num_threads(num_of_threads) default(none) shared(msh, solver,global_cmax)
  {
    // UNCOMMENT LINE BELOW IF NOT USING REDUCTION 
    double local_cmax = solver->q[0];
    
    #pragma omp for
    for(int i = 0; i < msh->NX * msh->NY; ++i) {
      if (solver->q[i] > local_cmax) {
        local_cmax = solver->q[i];
      }
    }
    //                              IMPLEMENTING CRITICAL PARALLELIZATION
    #pragma omp critical
    {
      if (local_cmax > global_cmax) {
        global_cmax = local_cmax;
      }
    }
  }
  end_time = omp_get_wtime();
  elapsed_time = end_time - start_time;
  printf("Time taken to find the infinite norm using Critical is: %f\n",elapsed_time);

  start_time = omp_get_wtime();
  #pragma omp parallel num_threads(num_of_threads) default(none) shared(msh, solver,global_amax)
  {
    double local_amax = solver->q[0];
    #pragma omp for
    for(int i = 0; i < msh->NX * msh->NY; ++i) {
      if (solver->q[i] > local_amax) {
        local_amax = solver->q[i];
      }
    }
    //                              IMPLEMENTING ATOMIC PARALLELIZATION

    if (local_amax > global_amax) {
  ...
  end_time = omp_get_wtime();
  elapsed_time = end_time - start_time;
  printf("Time taken to find the infinite norm using Atomic is: %f\n",elapsed_time);

   // End of parallel region
  printf("The infinite Norm of the solution vector is %f \n", global_rmax);
}
